...
float baseForceValue = 0;
float forceThreshold = 2.0;
...
const float MIN_FORCE = 4.0; // Minimum force threshold
...
void handleExternalPlayback() {
  ...
  if (currentPoint < (externalDataIndex-5)) 
  {
    ...
    if((playbackMode=="Assistive" || playbackMode=="Resistive") && targetTime >= 100)
      ...
      if (absVal1 >= absVal3 && absVal1 >= absVal2)
      {     
        tcurrentForce = loadcell1.get_units();
        netforce = abs(tcurrentForce - baseForceValue);
        if (netforce>level && playbackMode=="Resistive") 
        { 
          if(targetangle1>=0 && tcurrentForce>=0)
          {
            startTime = adjustedCurrentTime;
            sendExternalMovementCommand(currentPoint);
            currentPoint++;
          }
          else if(targetangle1<=0 && tcurrentForce<=0)
          {
            startTime = adjustedCurrentTime;
            sendExternalMovementCommand(currentPoint);
            currentPoint++;
       ...
       else if (netforce>level && playbackMode=="Assistive") 
        { 
          if(targetangle1>=0 && tcurrentForce>=0)
          {
            startTime = adjustedCurrentTime;
      ...
      else if(absVal2 >= absVal3)
      {
        tcurrentForce = loadcell1.get_units();
        Serial.print(tcurrentForce);
        Serial.print(" ");
        Serial.println(targetangle2);
        netforce = abs(tcurrentForce - baseForceValue);
        if (netforce>level && playbackMode=="Assistive") 
        { 
          if(targetangle2<=0 && tcurrentForce>=0)
          {
            startTime = adjustedCurrentTime;
            sendExternalMovementCommand(currentPoint);
            currentPoint++;
          }
          else if(targetangle2>=0 && tcurrentForce<=0)
          {
            startTime = adjustedCurrentTime;
            sendExternalMovementCommand(currentPoint);
            currentPoint++;
        ...
        else if (netforce>level && playbackMode=="Resistive") 
        { 
          if(targetangle2>=0 && tcurrentForce<=0)
          {
            startTime = adjustedCurrentTime;
      ...
      {
        tcurrentForce = loadcell2.get_units();
        netforce = abs(tcurrentForce - baseForceValue);
        if (netforce>level && playbackMode=="Resistive") 
        { 
          if(steps3<=0 && tcurrentForce>=0)
          {
            startTime = adjustedCurrentTime;
            sendExternalMovementCommand(currentPoint);
            currentPoint++;
          }
          else if(steps3>=0 && tcurrentForce<=0)
          {
            startTime = adjustedCurrentTime;
            sendExternalMovementCommand(currentPoint);
            currentPoint++;
...
void sendExternalMovementCommand(int pointIndex) {
  if (pointIndex == 0) {
    ...
    float targetAngle1 = externalPlaybackData[pointIndex].angle1;
    float targetAngle2 = externalPlaybackData[pointIndex].angle2;
    ...
    long maxSteps = max(max(abs(steps1/4), abs(steps2/4)), abs(steps3));
...
void recordDataPoint() {
  ...
  if (loadcell2.is_ready()) {
    ...
    float tnetForce = abs(tcurrentForce - baseForceValue);
    int speed;
...
void calibrateBaseForce() {
  ...
  for (int i = 0; i < samples; i++) {
    if (loadcell2.is_ready()) {
      totalForce += loadcell2.get_units();
    }
    delay(100);
  }
  
  baseForceValue = totalForce / samples;
  Serial.print("Base force value set to: ");
  Serial.println(baseForceValue, 2);
}
